...
export const setCartToOrder = catchAsync(async (req, res, next) => {
  ...
  const branchObj = await findByBranch(branch, {
...
export const getOrdersActive = catchAsync(async (req, res, next) => {
  ...
  let activeOrders = await findAllActivePast(order, {
    order_status: {
      [Op.in]: ['pending', 'confirmed', 'preparing', 'prepared', 'readytopickup']
    },
    customer_id: id
  });
  let pastOrders = await findAllActivePast(order, {
    order_status: {
      [Op.in]: ['rejected', 'delivered']
    },
    customer_id: id
  });
  ...
  for (let index = 0; index < activeOrders.length; index++) {
    ...
    element.order_items = await findAllOrderItem(order_item, {
      order_id: element.id
    });
  ...
  for (let index = 0; index < pastOrders.length; index++) {
    ...
    elementPast.order_items = await findAllOrderItem(order_item, {
      order_id: elementPast.id
    });
    for (let order_item = 0; order_item < elementPast.order_items.length; order_item++) {
      ...
      elementPostOrderItem.item = await findByOrderItem(item, {
        id: elementPostOrderItem.item_id
      });
      elementPostOrderItem.item_variant = await findByVariantId(item_variant, {
        id: elementPostOrderItem.item_variant_id
      });
...
export const checkUnreviewedOrder = catchAsync(async (req, res, next) => {
  ...
  let orderObj = await findByCustomerId(order, {
...
export const getAllUnreviewedOrder = catchAsync(async (req, res, next) => {
  ...
  let orderObjs = await findAllByCustomerId(order, {
    customer_id: customer_id,
    order_status: 'delivered',
    is_reviewed: 0
  });
