...
import {
  findAllCuisine
} from "../../services/cuisineService.js";
import {
  findByFilter
} from "../../services/restaurantDiscountService.js";
import {
  findVoucherBranch
} from "../../services/voucherService.js";
import {
  findAllBranchesByItem
...
import { getCustomerFromToken, objIsNotEmpty } from "../../libs/commomLibs.js";
...
export const getAllCuisines = catchAsync(async (req, res, next) => {
  ...
  for (let index = 0; index < cuisines.length; index++) {
    ...
    cuisines[index].totalbranches = (await findAllBranch(branch, {
      cuisine_id: {
        [Op.contains]: [element.id]
      }
    })).length;
  }
...
export const getAllFavouriteBranchs = catchAsync(async (req, res, next) => {
  ...
  let orderedRestaurantIds = await findByCustomerId(ordered_restaurant, customer_id);
  let results = _.map(_.filter(orderedRestaurantIds, {
    'is_favourite': 1
  ...
  for (let index = 0; index < branches.length; index++) {
    ...
    let destinations = [element.map_latitude + ',' + element.map_longitude];
    if (element.is_hasDiscount) {
      let restaurantDiscount = await findByFilter(restaurant_discount, {
        branch_id: element.id
      });
      element.discount = restaurantDiscount;
    }
    if (element.is_hasVoucher == 1) {
      let restaurantVoucher = await findVoucherBranch(voucher, {
        branch_id: element.id
      });
      element.voucher = restaurantVoucher;
    }
    let ratingObj = await findAllRating(ordered_restaurant, {
      branch_id: element.id
    });
    let reviews = await findAllReview(review, {
      branch_id: element.id
    });
    let favourites = await findAllRating(ordered_restaurant, {
      branch_id: element.id,
      is_favourite: 1
    });
...
export const getAllBranchsWithQueries = catchAsync(async (req, res, next) => {
  ...
  if (filters.vertical) {
    orAttributes.push({
      branch_type: filters.vertical
    });
  }
  if (filters.is_free_delivery) {
    orAttributes.push({
      is_free_delivery: filters.is_free_delivery
    });
  }

  if (filters.is_offer_available) {
    orAttributes.push({
      is_offer_available: filters.is_offer_available
    });
  }
  if (filters.is_online_payment) {
    orAttributes.push({
      is_online_payment: filters.is_online_payment
    });
  }
  ...
  if (filters.average_cost) {
    orAttributes.push({
      average_cost: filters.average_cost
    });
  }
  ...
  if (filters.cuisines) {
    ...
    orAttributes.push({
      cuisine_id: {
        [Op.contains]: filters.cuisines
      }
    });
  }
  if (filters.query) {
    const branchesWithItems = await findAllBranchesByItem(item, {
      name: {
      ...
      andAttributes.push({
        name: {
          [Op.or]: [{
            ...
            [Op.like]: "%" + filters.query.toLowerCase() + "%"
          }]
        }
      });
    }
  ...
  if (filters.query) {
    const [searchLog, created] = await findOrCreateSearch(customer_search_log, {
      search_content: filters.query
    });
...
export const getAllBranchsForCustomer = catchAsync(async (req, res, next) => {
  ...
  for (let index = 0; index < cuisineObjs.length; index++) {
    ...
    cuisineObjs[index].totalbranches = (await findAllBranch(branch, {
      cuisine_id: {
        [Op.contains]: [elementCuisine.id]
      }
    })).length;
  }

  //To Get All Campaigns
  let campaignObjs = await campaignService.findAll(campaign, {
    status: 1,
    is_active: 1,
    is_showonfront: 1
  });
  ...
  let promotions = await promotionService.findAll(promotion, {
    status: 1
  });
  for (let index = 0; index < promotions.length; index++) {
    let elementPromotion = promotions[index];
    let restaurant_promotions = await promotionService.findAll(restaurant_promotion, {
      promotion_id: elementPromotion.id
    });
    ...
    for (let rindex = 0; rindex < restaurant_promotions.length; rindex++) {
      ...
      let promotionBranchObj = await commonService.findWithModelAndFilter(branch, {
        id: restaurant_promotion.branch_id
      });
      let origins = [req.query.lat + ',' + req.query.lng];
      let destinations = [promotionBranchObj.map_latitude + ',' + promotionBranchObj.map_longitude];
      if (promotionBranchObj.is_hasVoucher == 1) {
        let restaurantVoucher = await findVoucherBranch(voucher, {
          branch_id: restaurant_promotion.branch_id
        });
      ...
      for (let index = 0; index < promotionBranchObj.cuisine_id.length; index++) {
        ...
        let cuisineObj = await commonService.findWithModelAndFilter(cuisine, {
          id: elementValue
        });
        cuisines.push(cuisineObj);
      }
      let ratingObj = await commonService.findWithModelAndFilter(ordered_restaurant, {
        branch_id: restaurant_promotion.branch_id
      });
      let reviews = await commonService.findAllWithModelAndFilter(review, {
        branch_id: restaurant_promotion.branch_id
      });
      let favourites = await findAllRating(ordered_restaurant, {
        branch_id: restaurant_promotion.branch_id,
  ...
  let recommendedBranches = await findAllFilterRecommended(branch, {
    is_recommended: 1
  });
  ...
  for (let index = 0; index < recommendedBranches.length; index++) {
    ...
    let origins = [req.query.lat + ',' + req.query.lng];
    let destinations = [recommendedElement.map_latitude + ',' + recommendedElement.map_longitude];
    if (recommendedElement.is_hasDiscount) {
      let restaurantDiscount = await findByFilter(restaurant_discount, {
        branch_id: recommendedElement.id
      });
  ...
  let yourBranches = [];
  let whereYour = {};
  let customerObj = await getCustomerFromToken(token);
  if (objIsNotEmpty(customerObj)) {
    let orderedRestaurantIds = await commonService.findAllWithModelAndFilter(ordered_restaurant, {
      customer_id: customerObj.id
    });
    let results = _.map(orderedRestaurantIds, branchId => {
      return parseInt(branchId.branch_id)
    });
    whereYour = {
      id: {
        [Op.in]: results
    ...
    for (let index = 0; index < yourBranches.length; index++) {
      ...
      let origins = [req.query.lat + ',' + req.query.lng];
      let destinations = [elementYour.map_latitude + ',' + elementYour.map_longitude];
      let cuisines = [];
